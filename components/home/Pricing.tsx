...
export const Pricing: React.FC<PricingProps> = ({ isDarkMode }) => {
    ...
    return (
        <section id="pricing" className="relative py-20 md:py-40 px-4 md:px-6 max-w-7xl mx-auto z-20">
            ...
            >
                <div className="inline-flex items-center gap-2 mb-8 py-2 px-6 rounded-full border border-white/10 bg-white/5 text-[10px] uppercase tracking-[0.3em] font-bold text-gray-500 dark:text-dark-text-tertiary">
            ...
            <div className="grid lg:grid-cols-3 gap-6 md:gap-8">
                {packages.map((p, i) => (
                    ...
                    >
                        ...
                        <h3 className="text-xl md:text-2xl font-light mb-2 text-gray-900 dark:text-dark-text-primary">{p.name}</h3>
                        <p className="text-xs uppercase tracking-widest text-gray-500 dark:text-dark-text-tertiary font-bold mb-8">{p.tagline}</p>

                        <div className="space-y-4 mb-10 flex-1">
                            {p.features.map((f, fi) => (
                                <div key={fi} className="flex items-start gap-3 text-sm text-gray-600 dark:text-dark-text-secondary">
                        ...
                        <button className={`w-full py-4 md:py-5 rounded-full font-bold uppercase tracking-widest text-[10px] md:text-[11px] btn-glow flex items-center justify-center gap-3 ${p.popular
                            ...
                            : `border border-black/5 dark:border-white/5 hover:bg-black/5 dark:hover:bg-white/5 text-gray-900 dark:text-dark-text-primary`
                            }`}>
